...
public class UIManager : Singleton<UIManager> // 설정도 여기에 추가 생각
{
    ...
    [Header("오프라인보상 판넬")]
    [SerializeField] private UIOfflineReward _offlineRewardPopup;
    /*
    [Header("배수버튼 판넬")]
    ...
    */
    ...
    /// <summary>
    /// GameManager에서 방치 시간 계산 완료 후 호출하는 함수
    /// </summary>
    public void ShowOfflineReward(int minutes, double gold)
    {
        // 1. 인스펙터 누락 대비 에러 방어 코드
        if (_offlineRewardPopup == null)
        {
            Debug.LogWarning("[UIManager] 오프라인 보상 팝업이 인스펙터에 연결되지 않았습니다!");
            return;
        }

        // 2. 연결된 팝업의 ShowReward 함수에 데이터 전달!
        _offlineRewardPopup.ShowReward(minutes, gold); //(이 부분의 파라미터 빈칸을 채워주세요!)
    }
    ...
    */
    ...
    void TogglePanel(GameObject targetPanel)
    {
        ...
        Debug.Log($"[UI 디버그] 누른 창: {targetPanel.name} / 기존에 열려있던 창: {(currentPanel != null ? currentPanel.name : "없음")} / 닫기 상태: {isSave}");
        CloseAll();
        if (isSave) // 같은 버튼 다시 누르면 닫기
        {
            OpenSoloSkillBar();
            currentPanel = null; // 초기화
            Debug.Log("[UI 디버그] 창을 닫고 SoloActiveSkillBar를 켭니다.");
            return;
        }
        ...
        Debug.Log($"[UI 디버그] {targetPanel.name} 창과 activeSkill을 켭니다.");
    }
    void CloseAll() // 전에 켜진 패널 닫기 용 함수
    {
        ...
        if (UIItemManager.Instance != null) // 배수 버튼도 같이 닫기용
        {
            var groups = UIItemManager.Instance.GetSlotGroups(); // UIItemManager에 public Getter 필요
            foreach (var group in groups)
            {
                if (group.multipleButton != null)
                {
                    group.multipleButton.gameObject.SetActive(false);
                }
            }
        }
    }
    ...
    */
    public void ToggleMultiple()
    {
        ...
        GameObject btnObj = UIItemManager.Instance.GetCurrentGroupButton(currentPanel);
